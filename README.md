...
| **商品浏览** | 商品列表分页展示、下拉刷新、商品详情查看（图文、价格、库存、卖家信息）、搜索功能 |
| **商品发布** | 支持多图上传、实时表单校验、自动压缩图片 |
| **交易流程** | 一键下单、订单创建（原子扣减库存）、订单状态查询、订单列表 |
| **个人中心** | 用户资料展示、我的订单、我发布的商品、退出登录 |
| **网络交互** | 基于 OkHttp 的统一网络请求、JSON 数据解析、Cookie 自动持久化 |
| **UI 组件** | RecyclerView 列表、Material Design 风格、FloatingActionButton 快速发布、CardView 卡片布局 |
...
│   │   │   │   │   ├── OrderActivity.java          # 订单详情
│   │   │   │   │   ├── OrderListActivity.java      # 订单列表
│   │   │   │   │   └── MyProfileActivity.java      # 个人资料
│   │   │   │   ├── adapter/                 # 适配器
